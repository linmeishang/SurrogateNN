# This file defines the Train_ResNet50 function
#%%
...
def convolutional_block(X, f, filters, stage, block, s = 2):
    ...
    # Third component of main path 
    X = Conv1D(F3, kernel_size = 1, strides = 1, name = conv_name_base + '2c', kernel_initializer=glorot_uniform(seed =0))(X)
    X = BatchNormalization(name = bn_name_base + '2c')(X)

    ##### SHORTCUT PATH #### 
    X_shortcut = Conv1D(F3, kernel_size = 1, strides = s, name = conv_name_base + '1', kernel_initializer=glorot_uniform(seed =0))(X_shortcut)
...
def ResNet50(input_shape, classes):
    ...
    filters_stage_2 = [16, 16, 4*16]
    filters_stage_3 = [2*16, 2*16, 8*16]
    filters_stage_4 = [4*16, 4*16, 16*16]
    filters_stage_5 = [8*16, 8*16, 32*16]
...
def Train_ResNet50(X_train, Y_train, m, lr, optimizer, n_epoch, model_name, seed):
    ...
    model = ResNet50(input_shape = (X_train.shape[1], 1), classes = Y_train.shape[1])
