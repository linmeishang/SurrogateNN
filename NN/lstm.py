# This is the code of training a LSTM
#%%
import numpy as np
np.random.seed(44)
import pandas as pd
...
model_path = path+"LSTM"+ datetime.now().strftime("_%Y%m%d%H") 
...
layer_1, layer_2, layer_3 = 32, 0, 0 # specify your own numbers
m = 32
n_epoch = 200
lr = 0.001
...
if layer_2 == 0:
    rnn = LSTM(layer_1, activation='relu', return_sequences = False)(inputs) 
    # ruu = Dropout(dropout_rate)(rnn) 

else:
    if layer_3 == 0:
        rnn = LSTM(layer_1, activation='relu', return_sequences = True)(inputs)
        # ruu = Dropout(dropout_rate)(rnn) 
        rnn = LSTM(layer_2, activation='relu', return_sequences = False)(rnn)
        # ruu = Dropout(dropout_rate)(rnn) 
    else:
        rnn = LSTM(layer_1, activation='relu', return_sequences = True)(inputs)
        # ruu = Dropout(dropout_rate)(rnn) 
        rnn = LSTM(layer_2, activation='relu', return_sequences = True)(rnn)
        # ruu = Dropout(dropout_rate)(rnn) 
        rnn = LSTM(layer_3, activation='relu', return_sequences= False)(rnn)
        # ruu = Dropout(dropout_rate)(rnn) 
